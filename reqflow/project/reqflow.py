...
DEFAULT_DATASET = _env_path("REQFLOW_DATASET", "data/dataset.csv")
DEFAULT_GOLD = _env_path("REQFLOW_GOLD", "data/gold_original.json")
...
def evaluate_run(pred_items: List[dict], gold_map: Dict[int, dict], mode: str, threshold: float) -> Tuple[dict, pd.DataFrame]:
    ...
    - Micro F1 (aggregated TP/FP/FN across all tags)
    - Macro F1 (average of per-tag F1 scores)
    - Per-tag breakdown in DataFrame
    """
    ...
    tag_f1_scores: List[float] = []  # For Macro F1 calculation
    ...
    # Micro F1: computed from aggregated counts
...
def evaluate_comprehensive(pred_items: List[dict], gold_map: Dict[int, dict], threshold: float = 0.5) -> dict:
    """
    Compute comprehensive evaluation metrics as recommended:
    - Primary: Micro F1 (Relaxed, threshold=0.5)
    - Secondary: Macro F1 (Relaxed)
    - Additional: Micro F1 at threshold=0.8 (stricter)
    - Baseline: Exact Match F1
    """
    # Primary: Relaxed at specified threshold (default 0.5)
    summ_relaxed, df_relaxed = evaluate_run(pred_items, gold_map, mode="Relaxed", threshold=threshold)
    
    # Stricter: Relaxed at threshold 0.8
    summ_strict, _ = evaluate_run(pred_items, gold_map, mode="Relaxed", threshold=0.8)
    
    # Baseline: Exact match
    summ_exact, _ = evaluate_run(pred_items, gold_map, mode="Exact", threshold=1.0)
...
class MetricLineChart(QWidget):
    ...
    def plot_threshold_curve(self, df: pd.DataFrame, title: str):
        ...
        # Check if this is Exact mode (only one threshold value = 1.0)
        unique_thresholds = df["threshold"].unique()
        is_exact_mode = len(unique_thresholds) == 1 and unique_thresholds[0] == 1.0
        
        if is_exact_mode:
            # For Exact mode: show bar chart instead of line chart
            metrics = ["precision", "recall", "f1"]
            values = [df[df["metric"] == m]["value"].iloc[0] for m in metrics]
            colors = ["#2b90d9", "#27c08a", "#f59e0b"]  # Blue, Green, Amber
            
            bars = ax.bar(metrics, values, color=colors, width=0.6, edgecolor="#2a2f36")
            ax.set_title("Exact Match Metrics (no threshold sweep)", pad=12)
            ax.set_ylabel("Score")
            ax.set_ylim(0.0, 1.0)
            ...
            for bar, val in zip(bars, values):
                ax.text(bar.get_x() + bar.get_width()/2, bar.get_height() + 0.02,
                       f"{val:.3f}", ha="center", va="bottom", color="#eaeaea", fontsize=10)
        else:
            # For Relaxed mode: show line chart with threshold sweep
            ...
            ax.set_ylim(0.0, 1.0)
            ax.set_xlim(0.0, 1.0)
            # Place legend inside chart in lower-right corner (less data there)
            leg = ax.legend(loc='lower left', framealpha=0.9, 
                           facecolor='#1a1f26', edgecolor='#3a3f47')
            for text in leg.get_texts():
                text.set_color('#dbe2ea')
...
class ReqFlowApp(QMainWindow):
    ...
    def _build_ui(self):
        ...
        mode_label.setToolTip("Exact: spans must match exactly (tag, start, end)\nRelaxed: uses IoU + text similarity (recommended)")
        ...
        self.eval_mode.setToolTip("Relaxed mode recommended for LLM evaluation\n(handles boundary variance)")
        ...
        thr_label.setToolTip("Minimum score for a match in Relaxed mode.\n0.50 is the NER standard (recommended).\n0.80 is stricter.")
        row.addWidget(thr_label)
        ...
        self.eval_thr.setToolTip("0.50 recommended (NER standard)\n0.80 for stricter evaluation")
    ...
    def compute_evaluation(self):
        ...
        # Get primary evaluation at selected threshold
        ...
        comp = evaluate_comprehensive(pred_items, gold_map, threshold=thr)
        
        # Build comprehensive summary text
        run_name = self.current_run_dir.name if self.current_run_dir else ''
        
        # Line 1: Run info
        line1 = f"Run: {run_name} | Pred: {self.current_pred_label}"
        ...
        if mode == "Relaxed":
            # Show exact match baseline for comparison
            exact_f1 = comp['exact']['micro_f1']
            strict_f1 = comp['strict']['micro_f1']
            line3 = (
                f"▸ Comparison: Exact Match F1={exact_f1} | "
                f"Relaxed@0.8 F1={strict_f1}"
            )
        else:
            # In Exact mode, show relaxed metrics for comparison
            relaxed_f1 = comp['primary']['micro_f1']
            relaxed_macro = comp['primary']['macro_f1']
            line3 = (
                f"▸ Comparison: Relaxed@{comp['threshold']} Micro F1={relaxed_f1} | "
                f"Macro F1={relaxed_macro}"
            )
        
        # Line 4: Counts
        line4 = (
            f"▸ Counts: Common={summ['common_requirements']} | "
            f"TP={summ['TP']} | FP={summ['FP']} | FN={summ['FN']}"
        )
        
        self.eval_summary.setText(f"{line1}\n{line2}\n{line3}\n{line4}")
